...
# 末尾に足していく。**既存列の順序は変えない**（シートは列順で書き戻すので、
# 間に挿すと過去の行が1つずつずれる）。無い列は "" で埋まる。
#   原価 … フードとドリンクに分かれない POS 用（新Uレジの材料原価）。
#          分かれている POS は従来どおり フード原価／ドリンク原価 に入れ、
#          **同じ数字を両方に書かない**（合計と内訳を二重に持つと必ずずれる）。
